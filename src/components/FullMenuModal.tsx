            ...
            <div className="p-8 space-y-10">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {t.menu.specials.map((special: any, idx: number) => (
                  <div
                    key={idx}
                    className="bg-secondary/20 border-2 border-secondary p-4 rounded-xl flex flex-col items-center text-center"
                  >
                    <Calendar className="text-primary-dark mb-2" size={24} />
                    <h4 className="font-bold text-dark uppercase text-lg">
                      {special.title}
                    </h4>
              ...
              <div className="bg-white p-6 rounded-2xl border-2 border-dark shadow-sm">
                ...
                <div className="flex flex-wrap gap-2">
                  {t.menu.meats.items.map((meat: string, idx: number) => (
                    <span
                      key={idx}
                      className="bg-light px-3 py-1 rounded-full text-sm font-bold text-dark border-2 border-dark hover:bg-secondary transition-colors cursor-default"
                    >
                      {meat}
                    </span>
                  ))}
                  ...
                  <div key={i}>
                    ...
                    <ul className="space-y-4">
                      {cat.items.map((item: any, j: number) => (
                        <li
                          key={j}
                          className="group border-b border-dark/10 pb-2 border-dashed last:border-0"
                        >
